...
import {LinkOffer} from "@/components/index";
const Nav = () => {
  let pathname = usePathname();
  const [offer, setOffer] = useState(false);

  return (
    <nav id={"nav"} className={"lg:text-sm lg:leading-6 relative h-full bg-transparent lg:border-r border-gray-50/[0.06]"}>
          ...
          <li className={"mt-12 lg:mt-8"}>
            ...
            <ul className={"space-y-6 lg:space-y-2 border-l border-slate-800"}>
              {categoriesRoute.map((category, index) => {
                ...
                return (
                   <LinkOffer href={category.route} setCurrentSection={setCurrentSection}>
